/**
 * @fileOverview This file handles the functionality of the privly
 * posting button. When the button is enabled in the options page,
 * the button is displayed in the top-right corner of any editable
 * textarea. On clicking the button, a new message can be posted.
 */
/*global privlyUrlReceiptNode:false, pendingPost:false, safari:false, ls:true,  */
...
function addPrivlyButton() {
  ...
  span.style.background = "url(" + safari.extension.baseURI + "img/logo_16.png) no-repeat";
  ...
  span.addEventListener( "click", function() {
    ...
    if (active && !pendingPost &&  (getComputedStyle(div).getPropertyValue("opacity") > 0)) {
      safari.self.tab.dispatchMessage("privlyMessage", "newPost");
      privlyUrlReceiptNode = context;
    } else {
      safari.self.tab.dispatchMessage("privlyMessage", "showNotification");
    }
  });
}

/**
 * Displays the privly button if it is set visible in the options page
 *
 * @param evt the message event received
 *
 */
function privlyButtonStatus(evt) {
  if (evt.name === "privlyMessage" && evt.message !== undefined &&
      evt.message.body !== undefined && evt.message.body.PrivlyBtnStatus === true) {
        addPrivlyButton();
  }
}

// Attach the message listener
safari.self.addEventListener("message", privlyButtonStatus, false);

// Send the message to the background page to know the privly button status
safari.self.tab.dispatchMessage("privlyMessage", "PrivlyBtnStatus");
